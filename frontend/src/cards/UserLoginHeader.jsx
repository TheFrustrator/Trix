...
const UserLoginHeader = ({ isSlidebarOpen, setIsSlidebarOpen }) => {
  const navigate = useNavigate();
  const location = useLocation();

  // Find the active role based on the current URL pathname
  const currentRoleObj =
    loginRoles.find((r) => r.route === location.pathname) || loginRoles[0]; // Fallback to first role if path doesn't match

  const handleRoleChange = (roleItem) => {
    setIsSlidebarOpen(false);
    if (roleItem.route && roleItem.route !== location.pathname) {
    ...
    <div className="flex flex-row justify-between items-center px-8 py-4">
      ...
      <div className="flex items-center gap-3">
        <div className="px-5 py-2 bg-white rounded-xl shadow-sm border border-gray-200 text-sm font-semibold text-[#264067]">
          Login in as {currentRoleObj?.title?.replace("For ", "")}
        </div>
